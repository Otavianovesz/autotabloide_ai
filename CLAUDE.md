...
- **F13 — as TRÊS TRAVAS DERRUBADAS pelo dono (24/07/2026, registradas na ORDEM_F13 §2):** (1) **a marca d'água RASCUNHO deixa de ser lei** — exportar sai **limpo por padrão**; RASCUNHO vira opção explícita do dono (botão/atalho) em todas as portas (*motivo:* eram 9 portas carimbando, a aprovação era inalcançável P-05/P-06 e a Fábrica não tinha caminho de aprovar P-07 — todo cartaz estava preso). (2) **O trio da célula (RG-15) cai** — clique seleciona **só a peça clicada**; mover a célula inteira ganha gesto próprio (definido no Bloco C da F13, junto com a causa E-01). (3) **O pré-voo da validade deixa de bloquear** — a validade se autopreenche pela campanha/evento e o pré-voo só avisa. *Estado:* decisões vigentes; a implementação entra nos Blocos C/D de `docs/ORDEM_F13_RESGATE.md` — até lá o código antigo ainda carimba/bloqueia. Onde esta lista conflitar com bullets antigos (F8/F11 sobre RASCUNHO/aprovação, RG-15), **esta decisão manda**.
...
**Lei da F13 (§5.5 do selo do Bloco B, 25/07/2026) — Animação no Qt: `stop()` NÃO emite `finished`.** Quem precisa saber que uma animação terminou escuta `stateChanged → Stopped`, nunca `finished` — e `stop()` numa animação já parada não emite nada. Todo registro de "animação viva" precisa de prazo de autoencerramento como rede. (Achado do builder na caça da ordem invertida; conserto-raiz em `animacoes.py::_registrar`. Pendência do §5.5 para o Bloco E: verificar se esta lenda explica o segfault intermitente que a F12 tratou com a regra do DeferredDelete.)
